...
def main():
    ...
    # קבלת משלוחים מהקובץ
    deliveries = [
        deliver.Delivery1("שמעון הצדיק אלעד", 2, 8, 14),
        deliver.Delivery1("בן קיסמא אלעד", 3, 9, 15),
        deliver.Delivery1("שמעון בן שטח אלעד", 4, 10, 16),
    ]
    current_location = deliver.Delivery1("יהודה הנשיא 53, אלעד", 0, 0, 0)  # המרת המיקום הנוכחי לאובייקט Delivery1
...
def build_route_with_delays(graphmaps, delivery_queue, delay_heap):
    """
    ...
    # המרת המיקום הנוכחי והיעד לאובייקטים של Delivery1
    ...
    if not delay_heap.is_empty():
        ...
        for candidate_delivery in delivery_queue:
            ...
            if alternative_time is None:    
                ...
                continue
            if alternative_time <= travel_time or alternative_time <= min_delay:
