import numpy as np
from osmFile import osmnx as ox
...
def get_node_id(graphmap, lon, lat):
    return ox.distance.nearest_nodes(graphmap, lon, lat)
...
def set_coordinates(graph, addressess, count):
    ...
    num_clusters = count
    coords = [get_coordinates(addr) for addr in addressess]
    coords = np.array([c for c in coords if c is not None])
    filtered_addresses = [addr for i, addr in enumerate(addressess) if get_coordinates(addr) is not None]
    print(num_clusters)
    # חלוקה לאשכולות עם KMeans
...
if __name__ == "__main__":
    ...
    clusters_info = set_coordinates(G, addresses)
    # הצגת מידע
