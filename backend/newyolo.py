...
def define_roi(frame, lane_width, num_lanes, vehicle_length):
    frame_height, frame_width = frame.shape[:2]

    # חישוב המיקום ההתחלתי של ה־ROI במרכז התמונה
    total_road_width = lane_width * num_lanes
    roi_x1 = int((frame_width - total_road_width) / 2)
    roi_x2 = roi_x1 + total_road_width

    # הגובה של ה־ROI – לפי איזור עניין בגובה
    roi_y1 = int(frame_height * 0.45)  # בערך מאמצע התמונה ומטה
    roi_y2 = int(roi_y1 + (vehicle_length * 2))  # שולי הרכב
...
def analyze_traffic(video_path, output_file, lane_width, num_lanes, vehicle_length, lane_start_offset):
    ...
    with open(output_file, 'a', encoding='utf-8') as f:  # שינוי ל-'a' במקום 'w'
        while cap.isOpened():
            ...
            if not ret:
                break
            roi_x1, roi_y1, roi_x2, roi_y2 = define_roi(
                frame, lane_width, num_lanes, vehicle_length
            )
